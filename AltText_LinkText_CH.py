    ...
    def process_img_tag(img):
        ...
        if 'alt' in img.attrs:
            ...
            if alt_text:
                ...
                image_data['Pass/fail score'] = calculate_score_img(alt_text)  # CH was the same as for get_link_text_info!
            else:
...
def get_link_text_info(url):
    ...
    def process_a_tag(a):
        ...
        # Adds calculated pass/fail score based on specific criteria
...
def start_screen():
    try:
        website_url = request.args['website_url']
        table_type = request.args['table_type']
    except Exception as e: # if we don't have both values, yet we must be at the first page ...
        return render_template('both_tables.html', table_type='None', website_url="http://www.nytimes.com")
    ...
    if table_type == "alt":
        ...
        if image_info != None:
            ...
            msg =  website_url + " is invalid. Try again" # will prevent table display!
        ...
        # This happens if the table is still set to "Select a table type"
